"""
RAG Engine - Core logic for document retrieval and question answering with Ollama.
Uses Chroma vector store and bge-base-en embeddings.
"""
...
# Embeddings - bge-base-en via sentence-transformers
...
class RAGEngineOllama:
    ...
    Uses Ollama for LLM, Chroma for vector store, and bge-base-en for embeddings.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            chroma_persist_dir: Directory to persist Chroma database
        ...
        if chroma_persist_dir is None:
            chroma_persist_dir = os.path.join(documents_dir, ".chroma_db")
        ...
        try:
            print("Initializing RAG corpus with Chroma...")
    ...
    def _load_documents(self, additional_paths: Optional[List[str]] = None) -> List[Any]:
        ...
        if os.path.exists(self.documents_dir):
            ...
            for root, dirs, files in os.walk(self.documents_dir):
                # Skip Chroma DB directory
                if '.chroma_db' in root or 'onenote_copies' in root:
                    continue
    ...
    def _get_embeddings(self) -> HuggingFaceEmbeddings:
        """Get or create embeddings model using bge-base-en."""
...
"""
    ...
    def _create_retriever(self, additional_paths: Optional[List[str]] = None):
        """
        Create or recreate the retriever with current settings using Chroma.
        ...
        print(f"Created {len(splits)} text chunks for indexing")

        # Create vector store with Chroma
        embeddings = self._get_embeddings()
        
        # Clear existing database if recreating
        if os.path.exists(self.chroma_persist_dir) and not additional_paths:
            import shutil
            print(f"Clearing existing Chroma database at {self.chroma_persist_dir}")
        ...
        print(f"✓ Chroma vector store created with {len(splits)} chunks")
    ...
    def _to_bullets(text: str, min_items: int = 3, max_items: int = 10) -> str:
        ...
        # Remove boilerplate
        text = RAGEngineOllama._strip_boilerplate(text)
